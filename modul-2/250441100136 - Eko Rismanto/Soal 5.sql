...
-- Membuat ulang dengan aturan RESTRICT pada kedua Foreign Key
CREATE TABLE rekam_medis (
    id_rekam INT PRIMARY KEY,
    id_pasien INT,
    id_dokter INT,
    tanggal_periksa DATE,
    diagnosis TEXT,
    CONSTRAINT fk_pasien_final FOREIGN KEY (id_pasien) 
        REFERENCES pasien(id_pasien) ON DELETE RESTRICT,
    CONSTRAINT fk_dokter_final FOREIGN KEY (id_dokter) 
        REFERENCES dokter(id_dokter) ON DELETE RESTRICT
);
...
-- Membuat ulang dengan aturan RESTRICT
CREATE TABLE rekam_medis (
    id_rekam INT PRIMARY KEY,
    id_pasien INT,
    id_dokter INT,
...
(701, 101, 2, '2026-04-07', 'Gejala Influenza'),
(702, 102, 2, '2026-04-07', 'Pemeriksaan Rutin');
